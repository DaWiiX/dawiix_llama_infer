...
namespace op
{
    ...
    {
        // pos tensor
        auto status = check_tensor_with_dim(get_input(2), base::DeviceType::DeviceCPU,
                                            base::DataType::DataTypeInt32, 1, 1);
        if (!status)
        {
        ...
        status = check_tensor_with_dim(get_input(1), device_type_, dtype_, 1, kv_dim_);
        if (!status)
        {
        ...
        status = check_tensor_with_dim(get_input(0), device_type_, dtype_, 1, dim_);
        if (!status)
        {
