...
namespace op
{
    ...
    base::Status EmbeddingLayer::check() const
    {
        ...
        (
            input_tensor, base::DeviceType::DeviceCPU,
            base::DataType::DataTypeInt32, 
            1, 
            token_size
        );
        ...
        status = check_tensor_with_dim(get_weight(0), device_type_, dtype_, 2, vocab_size_, dim_);
        if (!status)
        {
        ...
        status = check_tensor_with_dim(get_output(0), device_type_, dtype_, 2, token_size, dim_);
        if (!status)
        {
