...
namespace model
{
    ...
    LLama2Model::LLama2Model(std::string token_path, std::string model_path, bool is_quant_model)
        : Model(base::ModelType::ModelTypeLLama2, std::move(token_path), std::move(model_path),
                is_quant_model) {}
    ...
    base::Status LLama2Model::init(base::DeviceType device_type)
    {
        ...
        if (device_type == DeviceType::DeviceCUDA)
        {
            cudaSetDevice(0);
                                      ...
                                      int &next) const
    {
        ...
        if (device_type_ == base::DeviceType::DeviceCPU && is_quant_model_)
        {
    ...
    void LLama2Model::create_param_quant_layers()
    {
        ...
        auto cpu_device_type = base::DeviceType::DeviceCPU;
        ...
        for (int32_t i = 0; i < config_->layer_num_; ++i)
        {
            ...
            pos = pos + dim * dim + wq->get_scales() * sizeof(float);
        }
        ...
        for (int32_t i = 0; i < config_->layer_num_; ++i)
        {
            ...
            pos = pos + config_->kv_dim_ * dim + wk->get_scales() * sizeof(float);
        }
        ...
        for (int32_t i = 0; i < config_->layer_num_; ++i)
        {
            ...
            pos += config_->kv_dim_ * dim + wv->get_scales() * sizeof(float);
        }
        ...
        for (int32_t i = 0; i < config_->layer_num_; ++i)
        {
            ...
            pos = pos + dim * dim + wo->get_scales() * sizeof(float);
        }
        ...
        for (int32_t i = 0; i < config_->layer_num_; ++i)
        {
            ...
            pos = pos + dim * hidden_dim + w1->get_scales() * sizeof(float);
        }
        ...
        for (int32_t i = 0; i < config_->layer_num_; ++i)
        {
            ...
            pos = pos + dim * hidden_dim + w2->get_scales() * sizeof(float);
        }
        ...
        for (int32_t i = 0; i < config_->layer_num_; ++i)
        {
            ...
            pos = pos + dim * hidden_dim + w3->get_scales() * sizeof(float);
        }
        ...
        else
        {
            ...
            cls_layer->set_weight(0, {config_->vocab_size_, dim}, this->raw_model_data_->weight(pos),
                                  cpu_device_type);
            pos = pos + config_->vocab_size_ * dim + cls_layer->get_scales() * sizeof(float);
        }
    ...
    void LLama2Model::init_mem()
    {
        ...
        tensor::Tensor input_tokens(base::DataType::DataTypeInt32, 1, true, alloc_cpu);
        tensor::Tensor input_embeddings(base::DataType::DataTypeFp32, 1, config_->dim_, true, alloc);
        tensor::Tensor sin_cache(base::DataType::DataTypeFp32, config_->head_size_ * config_->seq_len_,
                                 true, alloc);
        tensor::Tensor cos_cache(base::DataType::DataTypeFp32, config_->head_size_ * config_->seq_len_,
                                 true, alloc);
        ...
        tensor::Tensor rms_output(base::DataType::DataTypeFp32, config_->dim_, true, alloc);
        ...
        tensor::Tensor w1_output(base::DataType::DataTypeFp32, config_->hidden_dim_, true, alloc);
        tensor::Tensor w3_output(base::DataType::DataTypeFp32, config_->hidden_dim_, true, alloc);

        CHECK(insert_buffer(ModelBufferType::W1Output, w1_output));
        CHECK(insert_buffer(ModelBufferType::W3Output, w3_output));

        // kv cache
        tensor::Tensor key_cache(base::DataType::DataTypeFp32, config_->layer_num_, config_->seq_len_,
                                 config_->kv_dim_, true, alloc);
        tensor::Tensor value_cache(base::DataType::DataTypeFp32, config_->layer_num_, config_->seq_len_,
                                   config_->kv_dim_, true, alloc);
        ...
        // Wq query output
        tensor::Tensor query(base::DataType::DataTypeFp32, config_->dim_, true, alloc);
        CHECK(insert_buffer(ModelBufferType::Query, query));

        // Pos tensor
        tensor::Tensor pos_tensor(base::DataType::DataTypeInt32, 1, true, alloc_cpu);
        CHECK(insert_buffer(ModelBufferType::InputPos, pos_tensor));

        // Attention output
        tensor::Tensor attn(base::DataType::DataTypeFp32, config_->head_num_, config_->seq_len_, true,
                            alloc);
        CHECK(insert_buffer(ModelBufferType::ScoreStorage, attn));
        CHECK(insert_buffer(ModelBufferType::AttnOutput, query));

        // final forward output
        tensor::Tensor forward_output(base::DataType::DataTypeFp32, config_->vocab_size_, true, alloc);
        if (device_type_ == base::DeviceType::DeviceCUDA)
        {
            tensor::Tensor forward_output_cpu(base::DataType::DataTypeFp32, config_->vocab_size_, true,
                                              alloc_cpu);
                                                                          ...
                                                                          int32_t token_pos) const
    {
        ...
        float *key_cache_ptr =
            const_cast<float *>(get_buffer(ModelBufferType::KeyCache).ptr<float>(cache_offset));
        float *val_cache_ptr =
            const_cast<float *>(get_buffer(ModelBufferType::ValueCache).ptr<float>(cache_offset));
        ...
        tensor::Tensor key(base::DataType::DataTypeFp32, config_->kv_dim_);
        tensor::Tensor val(base::DataType::DataTypeFp32, config_->kv_dim_);
    ...
    op::EmbeddingOutput LLama2Model::embedding(const std::vector<int> &tokens) const
    {
        ...
        auto input_token_num =
            tensor::Tensor(base::DataType::DataTypeInt32, static_cast<int32_t>(tokens.size()));
                                           ...
                                           bool is_prompt) const
    {
        ...
        tensor::Tensor input(base::DataType::DataTypeFp32, config_->dim_);
    ...
    void LLama2Model::attention_mha(int32_t layer_idx, const tensor::Tensor &pos_tensor) const
    {
        ...
        // wo @ attention output
        tensor::Tensor attn_output = get_buffer(ModelBufferType::AttnOutput);
    ...
    void LLama2Model::feed_forward(int32_t layer_idx, const tensor::Tensor &input) const
    {
        ...
        STATUS_CHECK(
            llama_layers_->add_layer_->forward(input, get_buffer(ModelBufferType::AttnOutput), input));
