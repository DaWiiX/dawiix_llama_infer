...
TEST(test_add_cu, add1_nostream) {
  ...
  tensor::Tensor t1(base::DataType::DataTypeFp32, size, true, alloc_cu);
  tensor::Tensor t2(base::DataType::DataTypeFp32, size, true, alloc_cu);
  tensor::Tensor out(base::DataType::DataTypeFp32, size, true, alloc_cu);

  set_value_cu(static_cast<float*>(t1.get_buffer()->ptr()), size, 2.f);
  set_value_cu(static_cast<float*>(t2.get_buffer()->ptr()), size, 3.f);

  kernel::get_add_kernel(base::DeviceType::DeviceCUDA)(t1, t2, out, nullptr);
...
TEST(test_add_cu, add1_stream) {
  ...
  kernel::get_add_kernel(base::DeviceType::DeviceCUDA)(t1, t2, out, stream);
  cudaDeviceSynchronize();
  float* output = new float[size];
  cudaMemcpy(output, out.ptr<float>(), size * sizeof(float), cudaMemcpyDeviceToHost);
  for (int i = 0; i < size; ++i) {
