...
{
    std::string model_path = "../../utils/test.bin";
...
TEST(test_load, create_matmul)
{
     ...
     *                                   1
     */
    auto wq = std::make_shared<op::MatmulLayer>(base::DeviceType::DeviceCPU, config.dim,
    ...
    tensor::Tensor tensor(base::DataType::DataTypeFp32, config.hidden_dim, false, nullptr, in);
    tensor.set_device_type(base::DeviceType::DeviceCPU);

    tensor::Tensor out_tensor(base::DataType::DataTypeFp32, config.dim, false, nullptr, out);
    out_tensor.set_device_type(base::DeviceType::DeviceCPU);

    wq->set_input(0, tensor);
    wq->set_output(0, out_tensor);
    wq->set_weight(0, {config.dim, config.hidden_dim}, weight_data, base::DeviceType::DeviceCPU);
    wq->forward(); // 完成一个计算
